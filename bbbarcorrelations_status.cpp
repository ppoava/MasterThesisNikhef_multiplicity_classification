...
int main(int argc, char **argv)
{
	...
	Int_t id, idBeauty, MULTIPLICITY, nMPIs, sph, nJets, nEvents, beautiness;
	...
	for (int iEvent = 0; iEvent < nEvents; iEvent++)
	{
		...
		for (int iPart = 0; iPart < nPart; iPart++)
		{
			...
			if (!particle.isFinal())
				continue; // Skip if the particle is not in its final state
			...
			// Filling vectors - save everything (not just beauty)
			vID.push_back(id);
			vPt.push_back(pT);
			vEta.push_back(eta);
			vY.push_back(y);
		...
		if (vID.empty() || vPt.empty() || vEta.empty() || vY.empty() || vPhi.empty() || vCharge.empty() || vStatus.empty() || vMother1.empty() || vMotherID.empty())
			continue;
		tree->Fill();
	} // End of event loop
